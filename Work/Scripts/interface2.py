...
class MainWindow(QMainWindow):
    def __init__(self,
        ...
        self.__currUsGroupsTableContents=[] #все группы доступные для добавления в группы пользователя, содержание таблички справа от групп пользователя
        self.__currEqGroupsTableContents=[] #все группы доступные для добавления в группы единицы оборудования, содержание таблички справа от групп оборудования
       ...
       # self.usChangeSearchGroupButton.clicked.connect(self.search_groups_in_eq_search_groups)
        self.show()
    def search_groups_in_eq_search_groups(self):
        if self.eqChangeGroupByName.text()!='':
            ...
            for i in self.__allGroups.keys():
                if i.find(self.eqChangeGroupByName.text())<0:
                    self.__currEqGroupsTableContents.remove(i)
    ...
    def refresh_all_eq_search_groups(self):
        self.eqSearchAllGrTableView.clearSpans()
        self.__currEqGroupsTableContents=sorted(self.__allGroups.keys())
        data_frame = pd.DataFrame(self.__currEqGroupsTableContents,
                                  columns=["Название групп"],
                                  index=[i for i in range(len(self.__currEqGroupsTableContents))])
        model = TableModel(data_frame)
        self.eqSearchAllGrTableView.setModel(model)
    def refresh_all_us_search_groups(self):
        self.usSearchAllGrTableView.clearSpans()
        self.__currUsGroupsTableContents=sorted(self.__allGroups.keys())
        data_frame = pd.DataFrame(self.__currUsGroupsTableContents,
                                  columns=["Название групп"],
                                  index=[i for i in range(len(self.__currUsGroupsTableContents))])
        model = TableModel(data_frame)
    ...
    def add_equipment(self):
        ...
        if self.heightSpinBox.value() == 0 or self.heightSpinBox.value() == -1:
            height = -1
        if code_error == -1 and self.__equipment_list.get_equipment_by_coordinates(from_left, height) is not None and from_left!=-1 and height!=-1:
            code_error = 9
        if code_error == -1 and self.__equipment_list.get_equipment_by_coordinates(from_left, height) is not None and from_left!=-1 and height!=-1:
            code_error = 9
        if code_error == -1:
    ...
    def setEqInfo(self):
        ...
        if self.__eqFoundTableContents[self.__eqnum][4] == '':
            self.__eqFoundTableContents[self.__eqnum][4] = '--'
        if self.__eqFoundTableContents[self.__eqnum][4] != '--':
    ...
    def searchEq(self):
        ...
        if self.eqsearchByGroupLineEdit.text() !="" and self.eqsearchByGroupLineEdit.text() in self.__allGroups.keys():
            ...
            print(self.__equipment_list.get_equipment_by_group(int(self.__allGroups[self.eqsearchByGroupLineEdit.text()])))
            for i in self.__equipment_list.get_equipment_by_group(self.__allGroups[self.eqsearchByGroupLineEdit.text()]):
                print(i)
                x = ""
                y = ""
                if i.x == -1:
